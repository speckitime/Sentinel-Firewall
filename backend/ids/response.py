...
class ResponseEngine:
    ...
    async def evaluate_alert(self, alert: dict) -> None:
        ...
        if confidence >= threshold and auto_block:
            action = "block"
            try:
                await self._nft.add_to_set("inet sentinel_firewall", "blocked_ips", src_ip)
            except Exception:
                pass
        elif confidence >= 60:
            action = "rate_limit"
            try:
                await self._nft.add_to_set("inet sentinel_firewall", "rate_limited_ips", src_ip)
            except Exception:
                pass
        elif confidence >= 40:
            action = "alert"
    ...
    async def get_stats(self) -> dict:
        ...
        return {"total": total, "blocked": blocked, "alerted": alerted,
